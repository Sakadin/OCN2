...
class TruePositiveRateEvaluator(Evaluator):
    def evaluate_rule(self, rule):
        tc = rule.target_class
        dist = rule.curr_class_dist
        p_dist = rule.prior_class_dist
...
class PrecisionEvaluator(Evaluator):
    def evaluate_rule(self, rule):
        ...
        if tc is not None:
            ...
            target = bn.nanmax(dist)
        return target / dist.sum()


class FalsePositiveRateEvaluator(Evaluator):
    def evaluate_rule(self, rule):
        tc = rule.target_class
        dist = rule.curr_class_dist
        if tc is not None:
            target = dist.sum()-dist[tc]
        else:
            target = bn.nanmax(dist)
        return -target
...
class LRSValidator(Validator):
    ...
    def evaluate_rule(self, rule, _default=False):
        tc = rule.target_class
        dist = rule.curr_class_dist

        if self.default_alpha <= 1.0:
            sig = self.test_sig1(dist, rule.initial_class_dist, tc, self.default_alpha)
            return sig
        if self.parent_alpha <= 1.0 and rule.parent_rule is not None:
            expdist = rule.parent_rule.curr_class_dist
            alpha = self.parent_alpha
            return self.test_sig1(dist, expdist, tc, alpha)

        return -1

    def test_sig1(self, obsdist, expdist, tc, alpha):
        if tc is not None:
            x = np.array([obsdist[tc], obsdist.sum() - obsdist[tc]], dtype=float)
            y = np.array([expdist[tc], expdist.sum() - expdist[tc]], dtype=float)
        else:
...
class RandomBeamSearchAlgorithm(SearchAlgorithm):
    """
    Remember the best rule found thus far and monitor a fixed number of
    alternatives (the beam).
    """
    ...
    def filter_rules(self, rules):
        if self.beam_width <= len(rules):
            return random.sample(rules,self.beam_width)
        return rules


class WeightedRandomSearchAlgorithm(SearchAlgorithm):
    """
    Remember the best rule found thus far and monitor a fixed number of
    alternatives (the beam).
    """
    ...
    def filter_rules(self, rules):
        temp = math.ldexp(1.0, -1022)
        weights = []
        for rule in rules:
            rule.do_evaluate()
            rule.is_significant()
            if rule.quality < 0:
                weights.append(abs((1 - rule.sig) * (1 + rule.quality))+temp)
            else:
                weights.append(abs((1 - rule.sig) * rule.quality)+temp)
        norm = weights / np.linalg.norm(weights,ord=1)

        if self.beam_width <= len(rules):
            return np.random.choice(rules, self.beam_width, False, norm)
        return rules
...
class TopDownSearchStrategy(SearchStrategy):
    ...
    """
    def __init__(self, constrain_continuous=True, evaluate=True,union=True,rnd=False,rat=1):
    ...
    def find_new_selectors(self, X, Y, W, domain, existing_selectors):
        ...
        #rnd false - classical algorithm, true - generating families of rulesets
        #union  true - ascending false - descending
        ...
        if not self.rnd:
            if self.union:
                for i, attribute in enumerate(domain.attributes):
                    ...
                    if attribute.is_discrete:
                        ...
                        if attribute.name.find("#gain") != -1:
                            for val in np.unique(X[:, i]):
                                s1 = Selector(column=i, op=">=", value=val)
                                possible_selectors.extend([s1, s1])
                        elif attribute.name.find("#loss") != -1:
                            for val in np.unique(X[:, i]):
                                s1 = Selector(column=i, op="<=", value=val)
                                possible_selectors.extend([s1, s1])
                        else:
            ...
            if self.union:
                random.getstate()
                #print(random.sample(domain.attributes, int(len(domain.attributes) * self.rat)))
                tempor = random.sample(domain.attributes, int(len(domain.attributes) * self.rat))
                for i, attribute in enumerate(tempor):
                ...
                random.getstate()
                #print(random.sample(domain.attributes, int(len(domain.attributes) * self.rat)))
                #print(int(len(domain.attributes) * self.rat))
...
class Rule:
    ...
    def is_significant(self, *args, **kwargs):
        ...
        """
        self.sig = self.significance_validator.evaluate_rule(self, *args, **kwargs)
...
class CN2UnorderedLearner(_RuleLearner):
    ...
    def fit(self, X, Y, W=None):
        ...
        rule_list.extend(self.find_rules(X, Y, W, self.tar_klasa,
                                         self.base_rules, self.domain))
